(function () {
  ...
  const STATE_WORDS = ["brouillon", "publiee", "publie", "annulee", "annule"];
  ...
  function removeKnownSyntax(value) {
    const code = comparableText(extractCode(value));
    let text = comparableText(value);

    text = text.replace(/\b(individuel|indiv|ind|collectif|collective|coll|col)\b/g, " ");
    text = text.replace(/\b(lundi|mardi|mercredi|jeudi|vendredi|samedi|dimanche)\b/g, " ");
    text = text.replace(/\d{1,2}\s*(?:h|:)\s*\d{0,2}\s*(?:[-a]\s*\d{1,2}\s*(?:h|:)\s*\d{0,2})?/g, " ");
    if (code) text = text.replace(new RegExp(`\\b${escapeRegExp(code)}\\b`, "g"), " ");
    STATE_WORDS.forEach(word => {
