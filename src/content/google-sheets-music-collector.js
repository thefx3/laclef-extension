(function () {
  ...
  function normalizeRowsText(value) {
    return String(value || "")
      .replace(/\r\n/g, "\n")
      .replace(/\r/g, "\n")
      .replace(/^\n+|\n+$/g, "");
  ...
  function countRowsText(value) {
    return normalizeRowsText(value)
      .split("\n")
      .filter(line => cleanText(line) || line.includes("\t"))
      .length;
  }
  ...
  function batchLabel(batch) {
    const rowCount = countRowsText(batch.rowsText);
  ...
  function expandClipboardHtmlRows(html) {
    if (!html) return "";
      ...
      [...row.cells].forEach(cell => {
        ...
        for (let rowOffset = 0; rowOffset < rowspan; rowOffset++) {
          ...
          for (let columnOffset = 0; columnOffset < colspan; columnOffset++) {
            grid[targetRow][columnIndex + columnOffset] = rowOffset === 0 && columnOffset === 0 ? value : "";
          }
        }
    ...
    return looksLikeMusicRows(rowsText) ? rowsText : "";
  }
    ...
    try {
      if (navigator.clipboard?.read) {
        ...
        for (const item of items) {
          ...
          const rowsText = expandClipboardHtmlRows(html);
          if (rowsText) return rowsText;
        }
      }
    } catch {
      // Fallback below: plain text is enough for non-merged selections.
    }

    try {
      return normalizeRowsText(await navigator.clipboard.readText());
    } catch {
      return "";
    }
  }
  ...
  async function createModal(initialRowsText) {
      ...
      <div class="fx-laclef-modal fx-sheets-music-modal" role="dialog" aria-modal="true">
        ...
        <div class="fx-laclef-modal-body">
          ...
          <div class="fx-laclef-form-grid fx-laclef-form-grid-compact">
            ...
            <div class="fx-laclef-field">
              <label for="fx-sheets-date-begin">Debute le</label>
              <input id="fx-sheets-date-begin" type="date" value="${escapeHtml(settings.dateBegin || "")}">
            </div>
            <div class="fx-laclef-field">
              <label for="fx-sheets-date-end">Termine le</label>
              <input id="fx-sheets-date-end" type="date" value="${escapeHtml(settings.dateEnd || "")}">
            </div>
            <div class="fx-laclef-field fx-laclef-field-full">
    ...
    const dateBeginInput = backdrop.querySelector("#fx-sheets-date-begin");
    const dateEndInput = backdrop.querySelector("#fx-sheets-date-end");
    ...
    backdrop.querySelector("#fx-sheets-add").addEventListener("click", async () => {
      ...
      const rowsText = normalizeRowsText(rowsInput.value);
