(function () {
  ...
  function buildProgramFromSlot(slot, settings, refs, suffixes) {
    ...
    const type = slot.lineCount > 1 ? "coll" : "individuel";
    ...
    const dateBegin = cleanText(settings.dateBegin || settings.date_begin);
    const dateEnd = cleanText(settings.dateEnd || settings.date_end);
    const label = [
      settings.instrument,
      type,
      slot.dayLabel,
        ...
        <tbody>
          ${items.map(item => `
            <tr>
              ...
              <td>${item.programmation ? `${escapeHtml(item.programmation.debut)} - ${escapeHtml(item.programmation.fin)}${item.programmation.date_begin || item.programmation.date_end ? `<br><span>${escapeHtml(item.programmation.date_begin || "")} - ${escapeHtml(item.programmation.date_end || "")}</span>` : ""}` : ""}</td>
  ...
  function createModal() {
        ...
        <div class="fx-laclef-modal-body">
          ...
          <section class="fx-music-tab-panel" data-panel="convert">
            <div class="fx-laclef-form-grid fx-laclef-form-grid-compact">
              ...
              <div class="fx-laclef-field">
                <label for="fx-music-date-begin">Debute le</label>
                <input id="fx-music-date-begin" type="date" value="${escapeHtml(settings.dateBegin || "")}">
              </div>
              <div class="fx-laclef-field">
                <label for="fx-music-date-end">Termine le</label>
                <input id="fx-music-date-end" type="date" value="${escapeHtml(settings.dateEnd || "")}">
              </div>
              <div class="fx-laclef-field fx-laclef-field-full">
                ...
                <div class="fx-laclef-note">Individuel : 1 ligne. Collectif : plusieurs lignes. La duree ne change pas cette regle ; les lignes vides d'un bloc fusionne comptent comme places.</div>
              </div>
            </div>
    ...
    const dateBeginInput = backdrop.querySelector("#fx-music-date-begin");
    const dateEndInput = backdrop.querySelector("#fx-music-date-end");
    ...
    [instrumentInput, placeSelect, individualTarifSelect, collectiveTarifSelect, dateBeginInput, dateEndInput, cloneInput].forEach(field => {
